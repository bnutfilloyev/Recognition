...
try:
    ...
    print(
        "onnxruntime is not detected, use fallback OpenCV backend. Try `pip install onnxruntime` or `pip install onnxruntime-gpu` according to hardware specification.")
...
class ONNXModelONNXRuntime(ONNXModel):
    def __init__(self, path):
        self.session = ort.InferenceSession(path)  # providers=['CPUExecutionProvider','CUDAExecutionProvider']
